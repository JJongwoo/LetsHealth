...
namespace Multi {
		...
		if (play_flag == true) {
			...
			for (int i = 0; i < theObjects.size(); i++) {
				...
				if (play_flag == true && is_red == true) {
					...
					if (theObjects.at(i).getYPos() <= y && gok < 80)
					{
						if (flag == true) {
							flag = false;
					...
					else if (theObjects.at(i).getYPos() >= yy && gok > 135)
					{
						if (flag == false) {
							SimpleAudioEngine::getInstance()->playEffect("Music/Perfect.mp3");
							flag = true;
							if (networkManager::getInstance()->gameTurn == MSGList::turn)
							{
								work_count++;
							}
						}
				...
				if (gok >= 135 && gok < 180) {
					...
					if ((endd - start) > 3) {
						...
						if (y == -1 && yy == -1) {
							...
							networkManager::getInstance()->succeedTurn();
						}
	...
	bool MultiGameScene::init()
	{
		...
		SetLabel->setString(intToString(work_count));
		...
		auto item1 = MenuItemImage::create(
			...
			CC_CALLBACK_1(MultiGameScene::doClose, this));
		...
		TurnSprite = Sprite::create("Images/YourTurn.png");
		TurnSprite->setScale(0.6f);
		TurnSprite->setPosition(Vec2(visibleSize.width*0.4, visibleSize.height*0.65));
		TurnSprite->setAnchorPoint(Vec2(0, 0));
		TurnSprite->setVisible(false);
		this->addChild(TurnSprite);


		/*sbtn = ui::Button::create("Images/BlueColor.png");
		sbtn->addClickEventListener(CC_CALLBACK_0(packet, this));
		sbtn->setPosition(Vec2(visibleSize.width*0.4, visibleSize.height*0.7));
		this->addChild(sbtn);*/
	...
	void MultiGameScene::doClose(Ref* pSender)
	{
		auto pScene = HelloWorld::createScene();
		Director::getInstance()->replaceScene(pScene);
	}

	void MultiGameScene::doRecall(Ref* pSender)
	{
		auto pScene = MultiGameScene::createScene();
		Director::getInstance()->replaceScene(pScene);
	}

	void MultiGameScene::update(float dt) {

		if (networkManager::getInstance()->gameTurn == MSGList::n_turn)
			asd = 0;
		if (networkManager::getInstance()->gameTurn == MSGList::turn) {
			play_flag = true;
			end1 = time(NULL);
			...
			if (end1 - start1 >= 2000) {
				if (asd == 0) {
					...
					asd = 3;
					asdasd = 1;
					networkManager::getInstance()->succeedFail();
				}
			}
		}
		...
		//박정환
		else if(networkManager::getInstance()->gameTurn == MSGList::n_turn)
		{
			
			TurnSprite->setVisible(false);
			turnFlag = true;
		}
		if (networkManager::getInstance()->gameTurn != MSGList::fail)
		ReadySprite->setVisible(false);


		if (networkManager::getInstance()->gameTurn == MSGList::turn && edu_flag == false) {
		...
		//네트워크 운동횟수 고정
		if (set_count <= 100 && play_flag == true) {
			SetLabel->setString(intToString(set_count));
		}
		if (work_count <= 3 && play_flag == true) {
			CountLabel->setString(intToString(work_count) + " / " + UserDefault::getInstance()->getStringForKey("ExerciseCount").c_str());
		}
		

		if (networkManager::getInstance()->gameTurn == MSGList::fail) {
			if (position_flag == true && time_flag == true) {
			...
			else if (position_flag == false && edu_flag == false)
			{
				GuideSprite->setVisible(false);
			}

			if (edu_flag == true && (gok >= 135 && gok < 180) && time_flag == false && position_flag == false)
			{
				//time_flag == false && guide_flag == false && (gok >= 135 && gok < 180) && edu_flag == true
		...
		if (networkManager::getInstance()->gameResult == MSGList::gameWin || networkManager::getInstance()->gameResult == MSGList::gameLose)
		{
			play_flag = false;
			end_time = time(NULL);
			if (end_flag == false) {
		...
		//networkManager::getInstance()->gameTurn == MSGList::turn
		if (networkManager::getInstance()->gameTurn == MSGList::turn) {
		//(int)end_interval - (int)start_interval == atoi(UserDefault::getInstance()->getStringForKey("ExerciseInterval").c_str())
		play_flag = true;
		rest_flag = true;
		interval_flag = false;
		...
		*/
		...
		if (position_flag==true) {
			//guide_flag == true && time_flag == true && interval_flag == false
			GuideSprite->setVisible(true);//setYourPosition
			ReadySprite->setVisible(false);
			ReadySprite->stopAllActions();
			GuideSprite->setTexture("Images/GuidePosition.png");
		...
		else if (time_flag == false && guide_flag == false && (gok >= 135 && gok < 180) && edu_flag == true && position_flag == false)
		{
			//time_flag == false && guide_flag == false && (gok >= 135 && gok < 180) && edu_flag == true
			GuideSprite->setVisible(false);//3   2    1
			ReadySprite->setVisible(true);
		...
		void MultiGameScene::showTurnSprite() {
			TurnSprite->setVisible(true);
			auto TurnFadeOut = FadeOut::create(0.8);
			auto TurnFadeIn = FadeIn::create(0.5);
			auto Turnseq = Sequence::create(TurnFadeOut, TurnFadeIn, nullptr);
			auto repeatTurn = RepeatForever::create(Turnseq);
			TurnSprite->runAction(repeatTurn);
		}
